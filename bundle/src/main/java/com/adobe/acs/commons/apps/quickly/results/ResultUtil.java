...
public final class ResultUtil {
    ...
    public static boolean isManuscript(final Resource resource) {
        return isAsset(resource)
                && StringUtils.endsWith(resource.getName(), ".txt");
    }

    public static boolean isWCMPath(final String path) {
        return StringUtils.startsWith(path, "/content/")
                && !isDAMPath(path)
                && !isUGCPath(path);
    }

    public static boolean isDAMPath(final String path) {
        return StringUtils.startsWith(path, "/content/dam/");
    }

    public static boolean isUGCPath(final String path) {
        return StringUtils.startsWith(path, "/content/usergenerated/");
    }

    public static boolean isToolsPath(final String path) {
        return StringUtils.startsWith(path, "/etc/");
    }

    public static boolean isNodeType(final Resource resource, final String... nodeTypes) {
    ...
    public static List<Resource> mergeAndDeDupe(final List<Resource> to, final List<Resource> from) {
